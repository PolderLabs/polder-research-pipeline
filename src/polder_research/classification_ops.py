...
from .classification import classify_text, select_provider
...
def _classification_config(root: Path) -> dict[str, Any]:
    try:
        value = yaml.safe_load((root / "knowledge-base" / "research.config.yaml").read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as exc:
        raise ValueError(f"cannot load classification config: {exc}") from exc
    classification = value.get("classification", {}) if isinstance(value, dict) else {}
    if not isinstance(classification, dict):
        raise ValueError("classification config must be a mapping")
    return classification


def _policy_for(kind: str, record: dict[str, Any], sources: dict[str, dict[str, Any]]) -> dict[str, Any]:
    related = [record]
    if kind == "segment":
        related.append(sources.get(str(record.get("source_id", "")), {}))
    elif kind in {"claim", "entity"}:
        related.extend(sources.get(str(source_id), {}) for source_id in record.get("source_ids", []))
    sensitivity = max(
...
def _input_for(kind: str, record: dict[str, Any]) -> str:
    """Return persisted replay text; source replay is intentionally metadata-only."""
    if kind == "source":
        return "\n".join(
            str(part) for part in (record.get("title"), record.get("source_type"), record.get("media_type"))
            if part
        )
    if kind == "segment":
        return str(record.get("text", ""))
    if kind == "claim":
        return str(record.get("statement", ""))
    if kind == "entity":
        return "\n".join(
            str(part)
            for part in (record.get("name"), record.get("description"), " ".join(record.get("aliases", [])))
            if part
        )
    raise ValueError(f"unsupported classification target kind: {kind!r}")
...
def evidence_targets(
    ...
    sources = {
        ...
        if (record := _load_json(path)) and isinstance(record.get("id"), str)
    }
...
def _taxonomy_identity(config: dict[str, Any]) -> tuple[str, str]:
    ...
    return _hash(json.dumps(taxonomy, sort_keys=True, separators=(",", ":"))), str(taxonomy.get("version", "1"))
...
def replay_existing(
    ...
    *,
    ...
    if resume_job_id and (limit is not None or provider is not None or tuple(kinds) != _KINDS):
        raise ValueError("resume uses the persisted target list; omit --kind, --limit, and --provider")
    ...
    if dry_run:
        preview = []
        for item in targets:
            selected_provider = provider or select_provider(config, item["kind"], item["policy_metadata"])
            blocked = selected_provider == "jev" and _is_sensitive_policy(item["policy_metadata"])
            preview.append({"kind": item["kind"], "id": item["id"], "provider": selected_provider, "sensitivity": item["policy_metadata"]["sensitivity"], "status": "blocked" if blocked else "pending"})
        return {"mode": "dry_run", "provider_override": provider, "taxonomy_sha256": taxonomy_sha256, "taxonomy_version": taxonomy_version, "selected": len(preview), "targets": preview}
    ...
    if resume_job_id:
        ...
        for item in targets:
            ...
            job["targets"].append({
                "target_kind": item["kind"], "target_id": item["id"], "input_sha256": _hash(item["text"][:maximum_chars]),
                "provider": selected_provider, "sensitivity": policy["sensitivity"], "personal_data": policy["personal_data"],
                "status": "blocked" if blocked else "pending", "attempts": 0, "classification_id": None,
                "error": "sensitive targets cannot be sent to Jev" if blocked else None,
            })
    lock = _job_lock(root, job["id"])
    try:
        job = _write_job(root, job)
        live_targets = {(item["kind"], item["id"]): item for item in evidence_targets(root, kinds=_KINDS)}
        for target in job["targets"]:
            ...
            if not item or _hash(item["text"][: int(config.get("max_input_chars", 12000))]) != target["input_sha256"]:
                target.update({"status": "failed", "error": "target is missing or its input changed", "updated_at": _now()})
                job = _write_job(root, job)
                continue
            if target["provider"] == "jev" and _is_sensitive_policy(item["policy_metadata"]):
                target.update({"status": "blocked", "error": "sensitive targets cannot be sent to Jev", "updated_at": _now()})
                job = _write_job(root, job)
                continue
            ...
            try:
                ...
                target.update({"status": "failed", "classification_id": None, "error": "classification invocation failed"})
            else:
                if result is None:
                    target.update({"status": "disabled", "classification_id": None})
                elif result.get("disposition") == "failed":
                    target.update({"status": "failed", "classification_id": result.get("id"), "error": result.get("error")})
                else:
                    target.update({"status": "completed", "classification_id": result.get("id")})
            target["updated_at"] = _now()
            job = _write_job(root, job)
        statuses = {target["status"] for target in job["targets"]}
        job["status"] = "completed" if not statuses.intersection({"pending", "running", "failed", "blocked"}) else "completed_with_issues"
        job = _write_job(root, job)
...
def comparison_report(repository_root: str | Path | None = None) -> dict[str, Any]:
    ...
    for path in sorted((root / ".research" / "classifications").glob("cls_*.json")):
        ...
        if not record or record.get("disposition") == "failed":
            continue
        key = tuple(str(record.get(field, "")) for field in ("target_kind", "target_id", "input_sha256", "taxonomy_sha256"))
    ...
    for predictions in groups.values():
        ...
        for index, first in enumerate(providers):
            for second in providers[index + 1 :]:
                ...
                category_agree += left.get("category") == right.get("category")
                tag_agree += set(left.get("proposed_tags", [])) == set(right.get("proposed_tags", []))
    return {
...
def replay_job_health(repository_root: str | Path | None = None) -> dict[str, Any]:
    ...
    for path in sorted(directory.glob("crj_*.v*.json")) if directory.exists() else []:
        ...
        if not match:
            malformed.append({"path": f".research/{_JOB_DIRECTORY}/{path.name}", "error": "invalid replay-job filename"})
            continue
    ...
    for job_id, path in sorted(latest.items()):
        ...
        try:
            ...
            malformed.append({"path": f".research/{_JOB_DIRECTORY}/{path.name}", "error": str(exc)[:300]})
            continue
...
def load_human_gold(path: str | Path) -> list[dict[str, Any]]:
    ...
    for number, line in enumerate(Path(path).read_text(encoding="utf-8").splitlines(), 1):
        ...
        if label["label_source"] != "human" or not isinstance(label["annotator"], str) or not label["annotator"].strip():
        ...
        if "taxonomy_sha256" in label and not _sha256(label["taxonomy_sha256"]):
            raise ValueError(f"gold label line {number} taxonomy_sha256 must be a 64-character lowercase SHA-256 when supplied")
        if "taxonomy_version" in label and (not isinstance(label["taxonomy_version"], str) or not label["taxonomy_version"]):
            raise ValueError(f"gold label line {number} taxonomy_version must be a non-empty string when supplied")
        ...
        if not isinstance(label["tags"], list) or not all(isinstance(tag, str) for tag in label["tags"]):
        ...
        if previous:
            ...
            same_label = previous["category"] == label["category"] and set(previous["tags"]) == set(label["tags"])
...
def _wilson_interval(successes: int, sample_count: int) -> dict[str, float] | None:
    ...
    return {"confidence_level": 0.95, "lower": max(0.0, centre - half_width), "upper": min(1.0, centre + half_width)}
...
def _calibration(values: list[tuple[float, int]]) -> dict[str, Any]:
    ...
    if not values:
        return {"supported": False, "sample_count": 0, "reason": "No prediction probabilities map directly to a gold decision."}
    ...
    for index, bucket in enumerate(bins):
        ...
        breakdown.append({"lower": index / 10, "upper": (index + 1) / 10, "count": len(bucket), "confidence": confidence, "accuracy": accuracy})
    return {
        "supported": True,
        "sample_count": sample_count,
        "brier_score": sum((probability - outcome) ** 2 for probability, outcome in values) / sample_count,
...
def _metrics(matches: list[tuple[dict[str, Any], dict[str, Any]]]) -> dict[str, Any]:
    labels_seen = sorted({str(label["category"]) for label, _ in matches} | {str(prediction.get("category")) for _, prediction in matches})
    ...
    for label, prediction in matches:
        ...
        probabilities = raw_category.get("probabilities", {}) if isinstance(raw_category, dict) else {}
        raw_choice = raw_category.get("choice") if isinstance(raw_category, dict) else None
        if isinstance(probabilities, dict) and isinstance(raw_choice, str) and isinstance(probabilities.get(raw_choice), int | float):
            category_calibration.append((float(probabilities[raw_choice]), int(label["category"] == raw_choice)))
    ...
    for value in labels_seen:
        tp = sum(str(label["category"]) == value and str(prediction.get("category")) == value for label, prediction in matches)
        fp = sum(str(label["category"]) != value and str(prediction.get("category")) == value for label, prediction in matches)
        fn = sum(str(label["category"]) == value and str(prediction.get("category")) != value for label, prediction in matches)
        category_rows[value] = {"support": sum(str(label["category"]) == value for label, _ in matches), **_prf(tp, fp, fn)}
    tag_names = sorted({tag for label, _ in matches for tag in label["tags"]} | {tag for _, prediction in matches for tag in prediction.get("proposed_tags", [])})
    tag_rows: dict[str, dict[str, Any]] = {}
    total_tp = total_fp = total_fn = 0
    for tag in tag_names:
        tp = sum(tag in label["tags"] and tag in prediction.get("proposed_tags", []) for label, prediction in matches)
        fp = sum(tag not in label["tags"] and tag in prediction.get("proposed_tags", []) for label, prediction in matches)
        fn = sum(tag in label["tags"] and tag not in prediction.get("proposed_tags", []) for label, prediction in matches)
        total_tp += tp
        total_fp += fp
        total_fn += fn
        tag_rows[tag] = {"support": sum(tag in label["tags"] for label, _ in matches), **_prf(tp, fp, fn)}
    macro_f1 = sum(row["f1"] for row in category_rows.values()) / len(category_rows) if category_rows else None
    return {
        "sample_count": len(matches),
        "category": {
            "accuracy": sum(label["category"] == prediction.get("category") for label, prediction in matches) / len(matches) if matches else None,
            "accuracy_wilson_95": _wilson_interval(
                sum(label["category"] == prediction.get("category") for label, prediction in matches), len(matches)
            ),
            "macro_f1": macro_f1,
            "per_label": category_rows,
            "confusion_counts": {actual: dict(sorted(values.items())) for actual, values in sorted(confusion.items())},
            "calibration": _calibration(category_calibration),
        },
        "tags": {"micro": _prf(total_tp, total_fp, total_fn), "per_tag": tag_rows, "calibration": _calibration(tag_calibration)},
    }


def evaluate_gold(
    gold: Iterable[dict[str, Any]], *, repository_root: str | Path | None = None, provider: str | None = None, split: str = "held_out"
) -> dict[str, Any]:
    ...
    for path in sorted((root / ".research" / "classifications").glob("cls_*.json")):
        record = _load_json(path)
        if not record or record.get("disposition") == "failed" or (provider and record.get("provider") != provider):
            continue
        predictions[(str(record.get("target_kind", "")), str(record.get("target_id", "")))].append(record)
    ...
    for label in labels:
        ...
        matching_input = [item for item in candidates if item.get("input_sha256") == label["input_sha256"]]
        matching_taxonomy = [
            item for item in matching_input
            if ("taxonomy_sha256" not in label or item.get("taxonomy_sha256") == label["taxonomy_sha256"])
            and ("taxonomy_version" not in label or item.get("taxonomy_version") == label["taxonomy_version"])
        ]
        if not matching_taxonomy:
            reason = "target_not_classified" if not candidates else "input_sha256_mismatch" if not matching_input else "taxonomy_mismatch"
            missing_reasons[reason] += 1
            missing.append({"target_kind": label["target_kind"], "target_id": label["target_id"], "reason": reason})
        else:
    ...
    stratified = {
        kind: _metrics([(label, prediction) for label, prediction in matches if label["target_kind"] == kind])
...
__all__ = ["comparison_report", "evaluate_gold", "evidence_targets", "load_human_gold", "replay_existing", "replay_job_health"]
