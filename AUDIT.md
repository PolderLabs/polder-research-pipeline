...
# Polder Research Pipeline — Latest Implementation Audit
...
Authoritative records (`.research/{events,tasks,runs,handoffs,sources,segments,claims,entities,gaps,conflicts,edges}/*.json`) remain **gitignored** under the local mode chosen for this audit round. The `.gitignore` flip that would track them is a one-way door: it commits the repository's runtime state to `origin/main` and is irreversible without rewriting history. Per advisory, the flip is deferred until a follow-up ships a recorded sample plus explicit collaborator sign-off.
...
# 22. Gitleaks integration currently fails

The secret-scan job is red.

The workflow references GITLEAKS_LICENSE.

The exact cause must be confirmed from the Actions logs before changing configuration.

Required outcome:

- keep secret scanning blocking;
- fix configuration/licensing/invocation;
- do not make CI green by silently disabling the scan.
...
The connector did not expose usable job logs for the failed workflow.

Therefore:

- failed CI state is confirmed;
- source-level defects above are confirmed from repository inspection;
- exact root cause of each individual failed CI job must still be verified from the Actions logs.

Do not weaken CI gates until those failures are understood.
