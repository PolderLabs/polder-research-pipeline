...
`.wolf/`, `.claude/`, and `.obsidian/` are machine-local state — all gitignored. `.research/` is runtime state under the repo root and is gitignored. Authoritative records include events, tasks, runs, handoffs, protocols, search executions, candidates, screening decisions, extractions, appraisals, sources, segments, claims, entities, gaps, conflicts, and evidence edges. Search-result exports are local inputs. `.research/reports/` contains generated review audit indexes; state and health snapshots are derived. Do not conflate these authority classes or claim local records are shared with collaborators.
...
For a systematic evidence review, follow `knowledge-base/00-home/research-methods.md` before acquisition. The manual note-intake steps below handle individual artifacts and do not establish systematic search coverage or independent screening.
...
- `.research/` stores authoritative local records and derived reports/snapshots under the repo root. Method and evidence records are atomically written; reports and snapshots are generated from those records. The gitignore policy means the local record set is not shared by default.
