...
1. **`vault_audit.py`** — Validates every vault note:
   ...
   - Reachability (every note reachable from a seed: `knowledge-base/index.md`, `knowledge-base/README.md`, `knowledge-base/AUDIT.md`, root `AGENTS.md`/`CLAUDE.md`, or top-level domain notes)
...
3. **`intake_register.py`** — Records raw material in `knowledge-base/90-inbox/manifest.md`:
...
4. **`new_note.py`** — Scaffolds a new durable note:
   - Wraps `knowledge-base/99-templates/<type>-template.md` (or generic template if missing)
   - Writes to the requested domain folder under `knowledge-base/`
