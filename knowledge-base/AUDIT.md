...
schemas and `SchemaRegistry` falls back to that bundled copy when a root has no
`schemas/` directory of its own. Isolated or partial workspaces also fall back to
the package schemas instead of losing record validation.

The same class of defect remains open for `vault-audit` and `frontmatter-fix`:
`src/polder_research/scripts/audit.py` and `frontmatter.py` load
`skills/obsidian-knowledgebase-curator/scripts/*.py` by repository path, and
`skills/` is not packaged. Those two commands therefore still fail on a
non-editable install. The documented installer uses an editable install, so the
shipped workspace is unaffected. Correcting this requires packaging the skill
scripts and is deliberately left for a separate change.
