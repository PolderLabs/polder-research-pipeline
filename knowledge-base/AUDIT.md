...
current directory. Commands that need workspace assets such as templates or
the Obsidian curator scripts resolve them from that workspace and return an
actionable error when the assets are missing. The wheel contains the Python
package and schemas; a full workspace still supplies its knowledge base,
